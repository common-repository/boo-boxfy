bbD.auto = {
    ...
    helpers: {
        ...
        get_dom: function () {
            var a = null,
                ...
                c = null,
                b = this.get_editor();
            e = (b.type == "RTE") ? jQuery(b.element.cloneNode(true)) : null;
            if (!e) {
        ...
        _close_tags: function (f, c) {
            ...
            b = f.replace(d, function (j, h, i) {
                ...
                return jQuery.grep(g, function (k) {
        ...
        getParent: function (d, c, b) {
            ...
            if (typeof(c) == "string") {
                ...
                c = function (f) {
                    ...
                    jQuery.each(a.split(","), function (g) {
    ...
    tag: {
        init_watch: function () {
            bbD.auto.control._last_count1 = jQuery.trim(bbD.auto.helpers.get_dom().html()).length;
            bbD.auto.control._last_count2 = jQuery.trim(bbD.auto.helpers.get_dom().text()).length;
        ...
        _watch: function () {
            var b = jQuery.trim(bbD.auto.helpers.get_dom().html()).length,
                a = jQuery.trim(bbD.auto.helpers.get_dom().text()).length,
        ...
        _setupLink: function (d, c) {
            jQuery("#boobox-links-content").html("");
            for (var e = 0; e < c.item.length; e++) {
                var j = c.item[e].name;
                var g = jQuery('<li><a href="">' + j + "</a></li>");
                g.click(function () {
                    bbD.auto.tag._toggleText(jQuery(this));
                    return false
                });
                var h = bbD.auto.helpers.get_dom();
                var f = jQuery("a:contains(" + j + ")", h);
                if (typeof(f[0]) != "undefined") {
                    g.addClass("selected")
                } else {
                    if (typeof(jQuery("span[class=bbused]:contains(" + j + ")", h)[0]) == "undefined") {
                        this._toggleText(g)
                    }
                }
                if (jQuery("#boobox-links-content li a")) {
                    jQuery("#boobox-links-content").show()
                }
                jQuery("#boobox-links-content").append(g)
            }
            if (c.item.length > 0) {
                if (typeof(jQuery("#markall")[0]) == "undefined") {
                    jQuery("#boobox-links-content").before(jQuery('<a href="javascript:void(0);" class="button" id="markall">' + bbD.auto.lang.markall + "</a>").click(function () {
                        jQuery("#boobox-links-content li").each(function () {
                            bbD.auto.tag._toggleText(jQuery(this), true)
                        });
                        return false
                    }))
                }
        ...
        _toggleText: function (h, a) {
            var j = h.text(),
                ...
                e = jQuery("a:contains(" + j + ")", c),
                g = jQuery("span[class=bbused]:contains(" + j + ")", c),
                f = c.html();
            var i = bbD.format,
                d = bbD.bid;
            if (a) {
    ...
    _saveNewlines: jQuery.browser.msie,
    nl: (jQuery.browser.msie ? "\r\n" : "\n"),
    nlrep: jQuery('<div><br class="boobox-bogus" /></div>').html()
};
